...
export default function Behavior() {
  ...
  return (
    <div className="mylo-page" style={{ color: 'var(--text-primary)' }}>
      ...
      {/* Explication */}
      <div style={{ padding: '12px 16px', borderRadius: 8, background: 'rgba(168,85,247,0.06)', border: '1px solid #A855F720', marginBottom: 20, fontSize: 12, color: 'var(--text-secondary)' }}>
        ...
        Après <strong style={{ color: 'var(--text-primary)' }}>20 flux minimum</strong>, la baseline est établie et Mylo détecte les écarts via Z-score.
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <button onClick={() => setFilter(p => ({...p, suspicious: !p.suspicious}))} style={{
          padding: '7px 14px', borderRadius: 8, border: `1px solid ${filter.suspicious ? '#EF4444' : 'var(--border-color)'}`,
          background: filter.suspicious ? 'rgba(239,68,68,0.1)' : 'transparent',
          color: filter.suspicious ? '#EF4444' : 'var(--text-tertiary)', cursor: 'pointer', fontSize: 13, fontWeight: 600,
        }}>
          🚨 Suspectes seulement
        </button>
        <button onClick={() => setFilter(p => ({...p, internal: !p.internal}))} style={{
          padding: '7px 14px', borderRadius: 8, border: `1px solid ${filter.internal ? '#F97316' : 'var(--border-color)'}`,
          background: filter.internal ? 'rgba(249,115,22,0.1)' : 'transparent',
          color: filter.internal ? '#F97316' : 'var(--text-tertiary)', cursor: 'pointer', fontSize: 13, fontWeight: 600,
        }}>
      ...
      <div className="mylo-table-scroll" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '160px 100px 120px 100px 100px 120px 80px 40px', padding: '12px 20px', borderBottom: '1px solid var(--border-color)', fontSize: 11, color: 'var(--text-muted)', fontWeight: 700, letterSpacing: '0.05em' }}>
        ...
        {loading ? (
          <div style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)' }}>Chargement des profils...</div>
        ) : baselines.length === 0 ? (
          ...
          return (
            <div key={b.ip_address}>
              <div onClick={() => toggleExpand(b.ip_address)}
                style={{ display: 'grid', gridTemplateColumns: '160px 100px 120px 100px 100px 120px 80px 40px', padding: '13px 20px', borderBottom: '1px solid var(--bg-primary)', cursor: 'pointer',
                ...
                {/* IP */}
                <div>
                  <div style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>{b.ip_address}</div>
                  {b.is_suspicious && <div style={{ fontSize: 10, color: '#EF4444' }}>⚠ SUSPECTE</div>}
                </div>

                {/* Type */}
                <span style={{ fontSize: 11, padding: '3px 8px', borderRadius: 20, background: b.is_internal ? 'rgba(249,115,22,0.1)' : 'rgba(100,116,139,0.1)', color: b.is_internal ? '#F97316' : 'var(--text-secondary)', height: 'fit-content' }}>
                ...
                {/* Flux */}
                <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{b.total_flows.toLocaleString()}</div>

                {/* Attaques */}
                <div style={{ fontSize: 13, color: b.attack_count > 0 ? '#EF4444' : 'var(--text-secondary)', fontWeight: b.attack_count > 0 ? 700 : 400 }}>
                ...
                {/* Baseline */}
                <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: b.baseline_established ? 'rgba(34,197,94,0.1)' : 'rgba(100,116,139,0.1)', color: b.baseline_established ? '#22C55E' : 'var(--text-tertiary)', height: 'fit-content' }}>
                  {b.baseline_established ? '✓ Prête' : `⏳ ${b.total_flows}/20`}
                </span>

                <ChevronDown size={14} color="var(--text-muted)" style={{ transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', alignSelf: 'center' }} />
              </div>

              {/* Détail expandé */}
              {isOpen && (
                <div style={{ padding: '20px', background: 'var(--bg-primary)', borderBottom: '1px solid var(--border-color)' }}>
                  {!d ? (
                    ...
                    <div>
                      ...
                      <div className="mylo-grid-cards" style={{ gap: 16, marginBottom: 16 }}>
                        {[
                          ...
                          <div key={label} style={{ background: 'var(--bg-card)', padding: '10px 14px', borderRadius: 8 }}>
                            <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 700, marginBottom: 4, textTransform: 'uppercase' }}>{label}</div>
                            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', fontFamily: 'monospace' }}>{value}</div>
                          </div>
                        ))}
                      </div>
                      ...
                      {d.typical_ports?.length > 0 && (
                        <div style={{ marginBottom: 12 }}>
                          <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 700, marginBottom: 6, textTransform: 'uppercase' }}>Ports habituels</div>
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                            {d.typical_ports.slice(0,15).map(p => (
                              <span key={p} style={{ fontSize: 12, padding: '2px 10px', borderRadius: 20, background: 'var(--border-color)', color: 'var(--text-secondary)', fontFamily: 'monospace' }}>:{p}</span>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      {Object.keys(d.typical_protocols || {}).length > 0 && (
                        <div style={{ marginBottom: 12 }}>
                          <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 700, marginBottom: 6, textTransform: 'uppercase' }}>Répartition protocoles</div>
                          <div style={{ display: 'flex', gap: 10 }}>
                            {Object.entries(d.typical_protocols).map(([proto, ratio]) => (
                              <div key={proto} style={{ textAlign: 'center' }}>
                                ...
                                <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{proto}</div>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                        <button onClick={() => resetBaseline(b.ip_address)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, border: '1px solid var(--border-color)', background: 'transparent', color: 'var(--text-tertiary)', cursor: 'pointer', fontSize: 12 }}>
